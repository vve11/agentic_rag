...
> "我做了一个面向 Agent 的论文研究 RAG Workbench：能自动发现候选论文、构建本地知识库、用 DeerFlow tool/subagent 调用 Agentic RAG，并用 60 条 strict golden + 40 条 real/stress 评测集验证检索、引用、拒答和回归质量。"
...
| 评测集 | strict golden 60 条，其中 45 条正例带 chunk-level 标注、15 条 no-evidence；real/stress 40 条 |
...
> 我把评测拆成两层。第一层是 retrieval-only，不调用大模型，只用 golden set 的 `relevant_paper_ids/relevant_chunk_ids` 和检索 top-k 做 ID 对比，所以可以稳定检查 paper recall、chunk recall、MRR、nDCG 和 FPR。第二层是 QA/no-judge，才调用模型生成答案，检查 citation existence、citation precision/recall、must-contain 和 no-answer success。引用评测还拆了 `relevant_chunk_ids` 和 `citation_chunk_ids`：前者测检索有没有找到核心证据，后者测最终 citation 是否直接支撑答案。三道防线：(1) 生成前 evidence selection 只给 LLM 精简证据；(2) prompt 强制使用 `[chunk:<id>]`；(3) `validate_citations` 剔除不在 selected evidence 的引用，no-evidence 走 abstain。
...
- 不要说“只跑了 demo question”——现在有 60 条 strict golden 和 40 条 real/stress。
